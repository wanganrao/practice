...
class View {
	...
	public function Set_CSS($filename){
		if (file_exists(ROOT . DS . $filename)){
			...
			$this->data['css'] = $this->data['css'] . '<link rel="stylesheet" type="text/css" media="screen" href="' .str_replace('\\', '/', $filename) . '">';
		}
	}
	
	public function Set_JS($filename){
		$this->data['js'] = $this->data['js'] . '<SCRIPT LANGUAGE="JavaScript" SRC="' . str_replace('\\', '/', $filename) . '"></SCRIPT>' . PHP_EOL;
	}
